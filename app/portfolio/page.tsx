...
import { PORTFOLIO_ACTIVITY_EVENT, type PortfolioActivity } from "@/lib/portfolio/activity-log";
...
function PortfolioConnectedView() {
  ...
  // which caused the chart to take different code paths on localhost vs deployed.
  ...
  const vaultActivityTotals = useMemo(() => {
    ...
    for (const a of historyActivities) {
      if (!a.vaultId || a.vaultId === "unknown") continue;
  ...
  const chartSeries = useMemo(() => {
    ...
    const validActivities = historyActivities.filter(
      (a) => a.vaultId && a.vaultId !== "unknown",
    );
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const dateLabel = new Date(ts).toLocaleDateString("en-US", { day: "numeric", month: "long" });

      dailyTotalActivity.push({ t: ts, date: dateLabel, value: cumulative });
      dailyFlowSeries.push({ t: ts, date: dateLabel, value: netFlow });
    }

    // 2. Decide the 'Total' series source based on available data quality
    let totalSeries;

    if (sampledPortfolioPoints.length >= 2) {
      // Best: Actual recorded NAV history via localStorage metrics (persists over time)
      totalSeries = sampledPortfolioPoints;
    } else if (validActivities.length > 0) {
      // Good: Simulate shape from local activity log, scaled to match current actual value
      const last = dailyTotalActivity[dailyTotalActivity.length - 1];
      const scale = last && last.value > 0 && totalValue > 0 ? totalValue / last.value : 1;
      totalSeries = dailyTotalActivity.map((d) => ({ ...d, value: d.value * scale }));
    } else {
      // Fallback: simple line from $0 to current value
      const dateFirst = new Date(firstDay).toLocaleDateString("en-US", { day: "numeric", month: "long" });
      const dateToday = new Date(todayStart).toLocaleDateString("en-US", { day: "numeric", month: "long" });
      totalSeries = [
        { t: firstDay, date: dateFirst, value: 0 },
        { t: todayStart, date: dateToday, value: totalValue },
      ];
    }
  ...
  }, [connected, walletAddress, refetch, refetchTransactions]);
